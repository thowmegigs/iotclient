...
import type { IPlan, IRole, IUser, Nullable } from "../../interfaces";
...
> = ({
  ...
  formState: { errors },
    ...
    const { autocompleteProps: autocompleteRoleProps } = useAutocomplete<IRole>({
      resource: "roles",
      defaultValue: queryResult?.data?.data?.role?.id,
    });
    return (
      <Dialog
        open={visible}
      ...
      >
        ...
        <DialogContent>
       {queryResult && queryResult.status=='loading'?<CircularProgress sx={{'mx':'auto'}} />:   <Box
            component="form"
            autoComplete="off"
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Autocomplete<any>
                    ...
                    getOptionLabel={(item) => {
                      return (
                        autocompleteRoleProps?.options?.find(
